...
def svr_1():
    ...
    #X_train, X_test, y_train, y_test = train_test_split(data,target, test_size=0.2, shuffle=False)
        ...
        plt.plot(time_axes, y_test, c='r', lw='0.1',label='predicted')
        plt.legend(title="SVR 2 Year Period")
        plt.savefig("SVR_2_year.png", bbox_inches='tight')
        plt.show()

        mae_value = float("{:.4f}".format(metrics.mean_absolute_error(y_test, prediction)))
        mse_value = float("{:.4f}".format(metrics.mean_squared_error(y_test, prediction)))
    ...
    with open(r'C:/Users/morga/Desktop/FINAL YEAR 2019/Project - Research Phase/ProjectData/Results/SVR_full_dataset/SVR_results.txt', "w") as f:
        x_i =1
